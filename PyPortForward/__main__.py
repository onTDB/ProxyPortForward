...
'''
 ...
 +-----------------------------+     +--------------+--------------+        +----------------+---------------+    +--------------------------------+
 | $ ssh -p 1022 user@1.2.3.4  |<--->|         1.2.3.4:5000        |<------>| IF 1: 5.6.7.8 | IF 2: 10.0.0.1 |      |<-->|       IF 1: 192.168.1.2        |
 | user@1.2.3.4's password:    |     +--------------+--------------+        +----------------+----------------+    +--------------------------------+
 | user@hostname:~$ whoami     |     | $ python ppf.py --server    |        | $ python ppf.py --port 5000    |    | 192.168.1.2:22(OpenSSH Server) |
 | user                        |     |                 --port 5000 |        |                                |    +--------------------------------+
 +-----------------------------+     +--------------+--------------+        +----------------+---------------+
'''
